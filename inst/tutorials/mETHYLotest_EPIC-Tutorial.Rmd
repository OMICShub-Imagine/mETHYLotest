...
# Note: You can click "Generate Structure Only" in the UI to create 
# the folder structure and config without running the full pipeline immediately.
...
|---|---|---|
| Pre-QC plots | `Results/QC_Raw/` | Raw beta distributions, density plots, and Recharts JSONs |
| Post-norm QC | `Results/QC_Normalised/` | Normalised beta distributions |
| DMP results | `Results/DMP/` | CSV (+ optional Excel) + Volcano/Distribution web JSON per comparison |
...
| Performance | `Results/Pipeline_Performance.csv` | Per-step timing and RAM (optional Excel `.xlsx` via `export_excel = TRUE`) |
...
│
└── Results/
    ├── project_config.R           (contains project_type="EPIC" & export settings)
    ...
    │   └── pipeline_results.rds
    │
    ├── QC_Raw/ (plots & Recharts dynamic web JSONs)
    ├── QC_Normalised/
    ├── QC_Combat/
    ├── CellType/
    ├── DMP/ (CSV result tables & volcano/distribution JSONs)
    ├── Episignatures/
    ├── DMP_Reports/
    ├── Validation/
    ├── DMR/
...
1. Recovers `myLoad.rds` (raw data) if previously generated by the main pipeline (lightning fast).
2. Merges your raw data with raw internal controls.
3. Automatically applies **BMIQ Normalization** and **ComBat Batch Correction** (if a control group is present) to the combined dataset to ensure perfect alignment.
4. Converts to M-values.
5. Computes per-probe Z-scores against the control distribution.
6. Reports a **Global Score** per signature per sample.
